...
def get_splits_player(id_player):
    ...
    # if table is None:
    #     i = 0
    #     while table is None or i < 10:
    #         url = 'http://www.espn.com/nba/player/splits/_/id/' + str(
    #             id_player)
    #         r = requests.get(url, verify=False)
    #         soup = bs4.BeautifulSoup(r.text, 'html.parser')
    #         table = soup.find(class_="Table__TBODY")
    #         i += 1
    # rows = table.find_all(class_=["total", "oddrow", "evenrow"])

    # rows = iter(rows)
    # for row in rows:
    #     data = [td.text for td in row.find_all('td') if td.text]
    #     name_key = data[0].lower().strip().replace('road', 'away').replace(
    #         '.', '')
    #     result[name_key] = data[1:]
    # return result
